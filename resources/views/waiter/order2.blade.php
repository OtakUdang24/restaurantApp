...
<div class="row clearfix">
      ...
      <div class="card">
          ...
          <div class="body">
            ...
            <div class="form-group form-float">
              ...
              <div class="form-group form-float{{ $errors->has('keterangan') ? ' error' : '' }}">
                  <div class="form-line">
                    <textarea name="keterangan" rows="4" id="keterangan" class="form-control no-resize" required="" placeholder="Please type what you want..."></textarea>
                  </div>
                  @if($errors->has('no_meja'))
                  <label id="name-error" class="error" for="keterangan">{{$errors->first('keterangan')}}</label>
                  @endif
              </div>
              <div class="form-group form-float">

                  <div class="form-line{{ $errors->has('pesenan') ? ' error' : '' }}">
                    <select autofocus id="no_meja" required class="selectpicker form-control show-tick" data-live-search="true" name="no_meja">
                      <option value="">--No Table--</option>
                      @foreach ($noMeja as $value)
                      <option value="{{$value->noMeja}}">{{$value->noMeja}}</option>
  ...
  <option value="5">Japan</option>
                  ...
                  @if($errors->has('pesenan'))
                  <label id="name-error" class="error" for="pesenan">{{$errors->first('pesenan')}}</label>
                  @endif
              </div>
